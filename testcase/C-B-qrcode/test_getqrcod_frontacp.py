...
#billNo="666813820913460515248912605736"
class GetOnceCode(unittest.TestCase):
    ...
    def test_a_getOnceQRCode(self):
        ...
        ##二维码的高级用法
        QRCodeWithLogo.getQRCode(QRCodeWithLogo(),response.json().get("billQRCode"),"onceqrcode.png","nfc.png")
        Image.open("onceqrcode.png").resize((360,360)).show()

    def test_b_QRCodeQuery(self):
        """  发起银联二维码支付结果查询，ACP银联二维码渠道查询成功 """
        ...
        print(response.text)
        self.assertEqual(response.json().get("errCode"),"SUCCESS","支付成功查询失败")
    ...
    def test_c_QRCodeRefund(self):
        ...
        print(refunddict)
        print(response.text)
        self.assertEqual(response.json().get("errCode"), "SUCCESS","支付宝银联渠道退款失败")
