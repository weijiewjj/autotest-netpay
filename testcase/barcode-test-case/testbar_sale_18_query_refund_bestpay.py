...
class TestPayVoidCheck(unittest.TestCase):
    ...
    def test_b_BarQuery(self):
        ''' 翼支付直连条码支付查询'''
        queryload = "{\"merOrderId\":\"" + self.mer_orderId + "\",\"mid\":\"898310077771111\", \
    \"msgSrc\":\"NETPAY\",\"msgType\":\"query\",\"requestTimestamp\":\""+self.now_time+"\",\"tid\":\"00000001\"}"
        headers = {
            'Content-Type': "application/json",
            'cache-control': "no-cache",
            'Postman-Token': "c10fac27-b245-4d3b-b52d-45e2d9790517"
        ...
        queryloadDict=SignDispose.getSignDict(SignDispose(), json.loads(queryload), self.signkey, "MD5")
        response = requests.request("POST", self.url, data=str(queryloadDict),headers=headers)
        print(response.text)
        self.assertIn(response.json().get("errCode"), "SUCCESS",
                      "void fail" + response.json().get("errCode") )

    def test_c_BarRefund(self):
        ''' 翼支付直连条码支付退款'''
        refundload = "{\"merOrderId\":\"" + self.mer_orderId + "\",\"mid\":\"898310077771111\", \
    \"msgSrc\":\"NETPAY\",\"msgType\":\"refund\",\"refundAmount\":\"1\",\"requestTimestamp\":\""+self.now_time+"\",\"tid\":\"00000001\"}"
        headers = {
            'Content-Type': "application/json",
            'cache-control': "no-cache",
            'Postman-Token': "c10fac27-b245-4d3b-b52d-45e2d9790517"
        ...
        refundDict=SignDispose.getSignDict(SignDispose(), json.loads(refundload), self.signkey, "MD5")
        response = requests.request("POST", self.url, data=str(refundDict),headers=headers)
        print(response.text)
        self.assertIn(response.json().get("errCode"), "SUCCESS",
                      "refund fail" + response.json().get("errCode") )